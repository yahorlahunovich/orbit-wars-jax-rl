...
from src.stage_classifier_model import predict_stage
...
def effective_cfg(state: GameState, base: dict[str, Any]) -> dict[str, Any]:
    ...
    # --- Predict game stage and adjust strategy ---
    try:
        my_planets_list = [p for p in state.planets if p.owner == me]
        enemy_planets_list = [p for p in state.planets if p.owner == 1 - me]
        neutral_planets_list = [p for p in state.planets if p.owner == -1]

        my_ships = sum(p.ships for p in state.planets if p.owner == me) + \
                   sum(f.ships for f in state.fleets if f.owner == me)
        enemy_ships = sum(p.ships for p in state.planets if p.owner == 1 - me) + \
                      sum(f.ships for f in state.fleets if f.owner == 1 - me)

        my_planets = len(my_planets_list)
        enemy_planets = len(enemy_planets_list)
        neutral_planets = len(neutral_planets_list)

        my_prod = sum(p.production for p in my_planets_list)
        enemy_prod = sum(p.production for p in enemy_planets_list)

        # Compute threat using aggregate_threats_by_planet
        ...
        if my_planets_list and enemy_planets_list:
            avg_dist_to_enemy = sum(math.hypot(p.x - e.x, p.y - e.y) for p in my_planets_list for e in enemy_planets_list) / (my_planets * enemy_planets)
        else:
            avg_dist_to_enemy = 100.0

        if my_planets_list and neutral_planets_list:
            avg_dist_to_neutral = sum(math.hypot(p.x - n.x, p.y - n.y) for p in my_planets_list for n in neutral_planets_list) / (my_planets * neutral_planets)
        else:
            avg_dist_to_neutral = 100.0

        features_dict = {
            'step_norm': state.step / 500.0,
        ...
        stage = predict_stage(features_dict)
        
        # Apply stage-specific adjustments
        if stage == 'DEFENSE':
            cfg["min_defense"] = 22
            cfg["launch_fraction"] = 0.50
            cfg["travel_time_max"] = min(cfg["travel_time_max"], 45.0)
            cfg["safety_margin"] = 4
            cfg["threat_horizon_turns"] = 110.0
            cfg["defense_reinforce_margin"] = 5
        elif stage == 'ATTACK':
            cfg["min_defense"] = 8
            cfg["launch_fraction"] = 0.85
            cfg["enemy_bonus"] = 25.0
            cfg["travel_time_max"] = max(cfg["travel_time_max"], 85.0)
            cfg["safety_margin"] = 1
            cfg["production_weight"] = 25.0
        elif stage == 'EXPANSION':
            cfg["min_defense"] = 8
            cfg["launch_fraction"] = 0.75
            cfg["production_weight"] = 45.0
            cfg["expansion_max_manhattan"] = 100
            cfg["safety_margin"] = 1
        elif stage == 'GROW':
            cfg["min_defense"] = 15
            cfg["launch_fraction"] = 0.50
            cfg["payoff_horizon"] = 140.0
            cfg["safety_margin"] = 2
    except Exception:
        pass
