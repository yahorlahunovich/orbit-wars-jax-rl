...
def joint_loss_fn(
    ...
    # Identify steps that had at least one valid source planet
    # (executed_mask is (N, P), so we reduce over P)
    step_mask = jnp.any(batch["executed_mask"], axis=-1)
    step_mask_f = step_mask.astype(jnp.float32)
    step_count = jnp.maximum(jnp.sum(step_mask_f), 1.0)

    old_log_prob = batch["old_log_prob"]
    adv_env = batch["advantages"]
    
    # Per-minibatch advantage normalization (masked)
    adv_mean = jnp.sum(adv_env * step_mask_f) / step_count
    adv_var = jnp.sum(step_mask_f * (adv_env - adv_mean)**2) / step_count
    ...
    # Value loss (masked)
    value_loss = jnp.sum(((batch["returns"] - out.value) ** 2) * step_mask_f) / step_count
