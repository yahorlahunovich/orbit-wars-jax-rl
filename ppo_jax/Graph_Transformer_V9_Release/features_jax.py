...
def extract_edge_features_v8_jax(state: JaxEnvState, player_id: Array) -> Array:
    ...
    Layout:
      geometry: dx, dy, dist, sin(angle), cos(angle), crosses_sun
      producer_roi: zero online placeholder
      source/target ship ratio
      bucket rough turns for 50/75/100
      bucket can-clear flags for 50/75/100
    """
    cur_turn = state.cur_turn
...
def extract_owned_nodes_jax(state: JaxEnvState, player_id: Array) -> Array:
    """Finds up to 16 active planet indices owned by the player, padded with -1."""
    cur_turn = state.cur_turn
    owners = state.future_timeline[:, cur_turn, 0]
...
def compute_edge_valid_mask_jax(
    ...
    The mask is not a strategy/capture-success filter. A target is allowed when
    the selected ship amount can be sent and the straight source->target segment
    is not blocked by the sun, an asteroid/comet, or another active planet.
    """
    cur_turn = state.cur_turn
    ...
    def check_slot(slot):
        ...
        def check_target(tgt):
            ...
            def check_opt(opt):
                ...
                # Self-edge option 0 is a no-op. This is critical: without it,
                # every owned planet is forced to launch every turn.
...
def compute_edge_valid_mask_raytrace_jax(
    ...
    This is O(N^3) and very slow — for evaluation/submission only.
    Use compute_edge_valid_mask_jax for training rollouts.
    """
    cur_turn = state.cur_turn
...
def state_to_graph_jax(state: JaxEnvState, player_id: Array) -> tuple[
    ...
    Array,  # owned_nodes (16,)
    Array,  # edge_valid_mask (60, 60, N_SHIP_OPTIONS) - simplified, does NOT ray-trace
]:
    """Top-level JAX-compiled features extractor. Uses simplified validity mask (no ray-tracing)."""
