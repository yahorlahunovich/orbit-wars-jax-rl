...
| v3 | Search + learned value function | 2026-05-28 | **1000+** (source notebook) | *(fill after local run)* | Heuristic candidate scoring + top‑K re-rank via 20‑tick forward simulation and GBC value function; falls back to heuristic if `value_gbc_trees*.py` is missing. Compute‑heavy. | `versions/lb_highest_1000_search_value/main.py` |
| v4 | Stage Classifier Heuristic | 2026-06-07 | *(to be submitted)* | Tied 3 - 3 (14 draws) vs v2 current heuristic in 20-game local bench | Uses an MLP classifier trained on 250 raw top-player replays to predict the upcoming game stage (ATTACK, DEFENSE, EXPANSION, GROW) over a 50-step lookahead window, dynamically tuning the heuristic's launch rates, reserve buffers, and ranges. | `versions/stage_classifier_heuristic_bot/main.py` |
